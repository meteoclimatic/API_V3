...
def _version_tuple(v):
    """Turn '3.8.0' or '5.0.2' into (3, 8, 0), ignoring any non-numeric
    suffix (e.g. '5.0.2b1' -> (5, 0, 2)). Avoids depending on distutils,
    ...
    for _p in v.split('.'):
        ...
        for _c in _p:
            if _c.isdigit():
                ...
                break
        _parts.append(int(_digits) if _digits else 0)
    return tuple(_parts)


if _version_tuple(weewx.__version__) < _version_tuple(REQUIRED_WEEWX):
...
class MeteoclimaticV3Thread(weewx.restx.RESTThread):
    ...
    _DATA_MAP = [
        ...
        ('WND',    'windSpeed', '',      '',    1),
        ('DGUST',  'windGust',  'Day',   'max', 1),
        ('MGUST',  'windGust',  'Month', 'max', 1),
        ('YGUST',  'windGust',  'Year',  'max', 1),
